/*
# Fix: include promotions.name in the advance_week() Phase 4 SELECT

Phase 4 generated upcoming events using `v_promo.name` but the cursor only selected
id/tier/fan_base. Adding `name` to the column list resolves the "record v_promo has
no field name" error. Drop and recreate advance_week() with the fix; everything else
stays identical to migration 0004b.
*/
...
BEGIN
  ...
  -- Phase 4: Generate upcoming events (FIX: includes name in SELECT)
    ...
    LOOP
      IF random() < LEAST(0.4, (v_fighter.current_skill + v_gym.reputation) / 300.0) THEN
        ...
        IF v_offer_count < 3 THEN
          ...
          IF v_opp IS NOT NULL THEN
            v_promo_for_offers := NULL;
            SELECT id, tier INTO v_promo_for_offers.id, v_promo_for_offers.tier FROM public.promotions
            WHERE owner_kind = 'ai' AND tier >= GREATEST(1, v_gym.tier) AND tier <= v_gym.tier + 1
            ORDER BY random() LIMIT 1;

            IF v_promo_for_offers.id IS NOT NULL THEN
              v_purse_base := v_promo_for_offers.tier * 5000 + GREATEST(0, (v_fighter.current_skill - 50) * 200);
              ...
              VALUES (v_gym.id, v_fighter.id, v_opp, v_promo_for_offers.id, NULL,
  ...
  -- Phase 9: Expire contracts and offers
