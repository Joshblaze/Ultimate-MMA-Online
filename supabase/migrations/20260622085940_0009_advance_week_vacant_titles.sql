/*
# Fix: Populate vacant championships

Currently advance_week() only schedules title defenses when
current_champion_fighter_id IS NOT NULL, so titles stay vacant forever.
Add a branch that crowns vacant titles: when a championship is vacant AND
the event falls on a title-fight roll, take the top 2 ranked contenders
and have them fight for the vacant belt.

Recreate advance_week() with the new vacant-title branch inside the
title-fights loop. Everything else identical to migration 0008.
*/
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_champion.champ_fighter IS NULL THEN
        -- VACANT TITLE: roll a title fight more often so belts get filled
        IF random() < 0.6 THEN
          -- Top 2 ranked contenders fight for the vacant belt
          v_contender_1 := NULL; v_contender_2 := NULL;
          SELECT fighter_id INTO v_contender_1 FROM public.rankings
          WHERE promotion_id = v_champion.promotion_id AND weight_class = v_champion.weight_class
          ORDER BY rank_position ASC LIMIT 1 OFFSET 0;
          SELECT fighter_id INTO v_contender_2 FROM public.rankings
          WHERE promotion_id = v_champion.promotion_id AND weight_class = v_champion.weight_class
          ORDER BY rank_position ASC LIMIT 1 OFFSET 1;
        ...
        END IF;
      ELSE
        -- DEFENSE: existing champion vs top contender
        IF random() < 0.4 THEN
      ...
      IF v_winner_id IS NOT NULL AND v_fighter_a IS NOT NULL AND v_fighter_b IS NOT NULL THEN
        ...
        ELSE
          -- New champion (either dethroned the old champ or won the vacant title)
          ...
          -- Gym credit if the new champion is managed by a player gym
