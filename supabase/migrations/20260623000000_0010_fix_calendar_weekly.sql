/*
# Fix: 1 tick = 1 in-game week (not 1 day)

advance_week() was incrementing current_day every tick and only bumping current_week
every 7 ticks, so the display calendar advanced 7x slower than tick_count.

Changes:
- Derive calendar fields from tick_count each tick (4 weeks/month, 12 months/year = 48 ticks/year)
- Align fighter aging to 48 ticks/year (was 52)
- Repair existing world_state calendar from current tick_count
*/

UPDATE public.world_state SET
  current_year  = floor(tick_count / 48) + 1,
  current_month = floor((tick_count % 48) / 4) + 1,
  current_week  = (tick_count % 4) + 1,
  current_day   = 1
WHERE id = 1;
