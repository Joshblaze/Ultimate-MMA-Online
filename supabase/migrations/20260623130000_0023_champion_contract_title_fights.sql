/*
# Champion contract renewal and title-only matchmaking

- Fighters holding a promotion title automatically renew their contract with that
  promotion when it would otherwise expire.
- Champions may only be booked in title fights (regular bouts and non-title offers
  exclude them).
*/

CREATE OR REPLACE FUNCTION public.fighter_holds_promotion_title(
  p_fighter_id uuid,
...
AS $$
  SELECT EXISTS (
    ...
    WHERE c.current_champion_fighter_id = p_fighter_id
      AND (p_promotion_id IS NULL OR c.promotion_id = p_promotion_id)
  );
$$;

CREATE OR REPLACE FUNCTION public.renew_champion_contract(
  p_fighter_id uuid,
  p_promotion_id uuid,
  p_tick int
)
...
DECLARE
  ...
  v_purse bigint;
BEGIN
  IF NOT public.fighter_holds_promotion_title(p_fighter_id, p_promotion_id) THEN
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION public.enforce_champion_title_fights()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER SET search_path = public
AS $$
BEGIN
  IF NEW.is_title_fight THEN
    RETURN NEW;
  END IF;
  ...
  IF EXISTS (
    SELECT 1
    FROM public.championships c
    WHERE c.current_champion_fighter_id IN (NEW.fighter_a_id, NEW.fighter_b_id)
  ) THEN
    RAISE EXCEPTION 'Champions may only compete in title fights';
  END IF;
...
DROP TRIGGER IF EXISTS enforce_champion_title_fights ON public.fights;
CREATE TRIGGER enforce_champion_title_fights
BEFORE INSERT ON public.fights
FOR EACH ROW EXECUTE FUNCTION public.enforce_champion_title_fights();
    ...
    IF v_fight.is_title_fight AND v_fight.championship_id IS NOT NULL THEN
      ...
      IF v_old_champ_id IS NOT NULL AND v_winner_id = v_old_champ_id THEN
        ...
        WHERE championship_id = v_fight.championship_id
          AND fighter_id = v_winner_id
          AND lost_at_week IS NULL;

        INSERT INTO public.news_items (week, type, title, body, fighter_id, promotion_id)
        VALUES (NEW.completed_at_week, 'title_defense',
          v_winner_name || ' retains the title',
          v_winner_name || ' successfully defended the championship at ' || OLD.name ||
            ' via ' || v_method || ' in round ' || v_round || '.',
          v_winner_id, OLD.promotion_id);
      ELSE
        IF v_old_champ_id IS NOT NULL THEN
          UPDATE public.title_history SET lost_at_week = NEW.completed_at_week
          WHERE championship_id = v_fight.championship_id
            AND fighter_id = v_old_champ_id
            AND lost_at_week IS NULL;
        ...
        VALUES (v_fight.championship_id, v_winner_id, NEW.completed_at_week, 0);
...
-- Repair any champions who lost promotion ties when a contract expired.
DO $$
DECLARE
  v_tick int;
  v_champ RECORD;
    ...
    LOOP
      ...
      IF v_championship_id IS NOT NULL THEN
        IF random() < LEAST(0.5, (v_fighter.current_skill + v_gym.reputation) / 250.0) THEN
          ...
          IF v_offer_count < 3 THEN
            ...
            SELECT r.fighter_id INTO v_opp
            FROM public.rankings r
            JOIN public.fighters opp ON opp.id = r.fighter_id
      ...
      IF random() < LEAST(0.4, (v_fighter.current_skill + v_gym.reputation) / 300.0) THEN
        ...
        IF v_offer_count < 3 THEN
          ...
          INTO v_promo_offer_id, v_promo_offer_tier
          FROM public.contracts c
          JOIN public.promotions p ON p.id = c.promotion_id
