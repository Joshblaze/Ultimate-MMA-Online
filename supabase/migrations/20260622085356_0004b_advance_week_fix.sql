/*
# Fix: add missing variable declarations in advance_week()

Adds declarations for v_fighter_a and v_fighter_b used during event-population.
Replaces the function with the corrected version.
*/

CREATE OR REPLACE FUNCTION public.advance_week2()
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  v_week := v_world.tick_count;
  v_new_tick := v_week + 1;
  v_new_day := (v_world.current_day % 7) + 1;
  IF v_world.current_day = 7 THEN
    v_new_week := v_world.current_week + 1;
    IF v_new_week > 4 THEN
      v_new_week := 1;
      v_new_month := v_world.current_month + 1;
      IF v_new_month > 12 THEN
        v_new_month := 1;
        v_new_year := v_world.current_year + 1;
      ELSE
        v_new_year := v_world.current_year;
      END IF;
    ELSE
      v_new_month := v_world.current_month;
      v_new_year := v_world.current_year;
    END IF;
  ELSE
    v_new_week := v_world.current_week;
    v_new_month := v_world.current_month;
    v_new_year := v_world.current_year;
  END IF;
  ...
  -- Phase 1: Age fighters each year (every 52 ticks)
  UPDATE public.fighters SET age = age + 1
  WHERE (v_new_tick % 52) = 0 AND retired = false;

  -- Phase 2: Train fighters (drift toward potential)
  ...
  -- Phase 4: Generate upcoming events
  FOR v_promo IN SELECT id, tier, fan_base FROM public.promotions WHERE owner_kind = 'ai' LOOP
    ...
    IF v_count < 1 THEN
      ...
      VALUES (v_promo.id, v_promo.name || ' #' || (v_new_tick + 1),
        v_new_tick + 1 + floor(random() * 2)::int, 'scheduled');
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    FOR v_wc IN SELECT name FROM public.weight_classes ORDER BY "order" LOOP
      ...
      WHERE promotion_id = v_events_to_process.promotion_id AND weight_class = v_wc.name AND retired = false
      ...
      IF v_fighter_a IS NOT NULL THEN
        ...
        WHERE promotion_id = v_events_to_process.promotion_id AND weight_class = v_wc.name
          AND retired = false AND id <> v_fighter_a
        ORDER BY random() LIMIT 1;
      END IF;
    ...
    LOOP
      IF random() < 0.25 AND v_champion.champ_fighter IS NOT NULL THEN
        v_opp := NULL;
        SELECT r.fighter_id INTO v_opp FROM public.rankings r
        WHERE r.promotion_id = v_champion.promotion_id AND r.weight_class = v_champion.weight_class
          AND r.fighter_id <> v_champion.champ_fighter
        ORDER BY r.rank_position ASC LIMIT 1;

        IF v_opp IS NOT NULL THEN
          v_fighter_a_skill := (SELECT current_skill FROM public.fighters WHERE id = v_champion.champ_fighter);
          v_fighter_b_skill := (SELECT current_skill FROM public.fighters WHERE id = v_opp);
          v_a_strength := v_fighter_a_skill + floor(random() * 25)::int;
          v_b_strength := v_fighter_b_skill + floor(random() * 25)::int;
          v_old_champ_id := v_champion.champ_fighter;

          IF v_a_strength >= v_b_strength THEN v_winner_id := v_champion.champ_fighter;
          ELSE v_winner_id := v_opp; END IF;
          ...
          v_commentary := jsonb_build_array(
            'Championship main event: ' || (SELECT name FROM public.fighters WHERE id = v_champion.champ_fighter) ||
          ...
          INSERT INTO public.fights (event_id, fighter_a_id, fighter_b_id, winner_id, method, round,
            commentary, weight_class, is_title_fight, championship_id, status, completed_at_week)
          VALUES (v_events_to_process.id, v_champion.champ_fighter, v_opp, v_winner_id, v_method, v_round,
          ...
          UPDATE public.fighters SET losses = losses + 1
            WHERE id IN (v_champion.champ_fighter, v_opp) AND id <> v_winner_id;

          IF v_winner_id = v_old_champ_id THEN
          ...
          ELSE
            ...
            UPDATE public.gyms SET champions_produced = champions_produced + 1
    ...
    LOOP
      IF random() < LEAST(0.4, (v_fighter.current_skill + v_gym.reputation) / 300.0) THEN
        ...
        IF v_offer_count < 3 THEN
          ...
          IF v_opp IS NOT NULL THEN
            v_promo.id := NULL; v_promo.tier := NULL;
            SELECT id, tier INTO v_promo FROM public.promotions
            WHERE owner_kind = 'ai' AND tier >= GREATEST(1, v_gym.tier) AND tier <= v_gym.tier + 1
            ORDER BY random() LIMIT 1;

            IF v_promo.id IS NOT NULL THEN
              v_purse_base := v_promo.tier * 5000 + GREATEST(0, (v_fighter.current_skill - 50) * 200);
              ...
              VALUES (v_gym.id, v_fighter.id, v_opp, v_promo.id, NULL,
                v_purse_base, v_new_tick + 1 + floor(random() * 2)::int, 'pending', v_new_tick);
  ...
  -- Phase 8: Update gym rankings by reputation
  v_rank := 1;
  ...
  -- Phase 9: Expire old contracts and offers
...
-- Rename to the canonical function name (drop the original broken one first)
DROP FUNCTION IF EXISTS public.advance_week();
ALTER FUNCTION public.advance_week2() RENAME TO advance_week;
